...
class Module(Classes.Vulnerable, Object):
    """ A basic part of the ship."""
    def __init__(self, hp, mass, image):
        Object.__init__(self, image=image)
...
class Weapon(Module):
    ...
    def _ballistic_get_predict_pos(self, locked):
        ...
        :return:
        """
        predict_pos = Object(A.blanc, 0, 0)
    ...
    def shot(self):
        ...
        if not self.blocked:
            ...
            shot = Projectile(self.bolt, self.rect.centerx,
    ...
    def _shot_laser(self):
        if self.aim(self.target) and self.target.get_dist() < self.range:
            gl = FX_Glow(self.rect, 1, 10, 5, (255, 0, 0))
            # self.target
